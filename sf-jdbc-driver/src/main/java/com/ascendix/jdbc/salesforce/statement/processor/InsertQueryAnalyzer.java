...
public class InsertQueryAnalyzer {
    ...
    private static final Pattern INSERT_STATEMENT = Pattern.compile("(?i)INSERT\\s+INTO\\s+(?<EntityName>\\w+)\\s*\\((?<FieldNames>.+)\\)\\s*VALUES\\s*\\((?<FieldValues>.+)\\)\\s*;?");
    ...
    public InsertQueryAnalyzer(String soql, Function<String, DescribeSObjectResult> objectDescriptor, Map<String, DescribeSObjectResult> describedObjectsCache) {
        this.soql = soql;
    ...
    public boolean analyse(String soql) {
        ...
        Matcher matcher = INSERT_STATEMENT.matcher(soql);
    ...
    private class SelectSpecVisitor extends SOQLDataBaseVisitor<Void> {
        ...
        public Void visitSOQLSubQuery(SOQLSubQuery soqlSubQuery) {
            ...
            InsertQueryAnalyzer subqueryAnalyzer = new InsertQueryAnalyzer(subquery.toSOQLText(), objectDescriptor, describedObjectsCache);
